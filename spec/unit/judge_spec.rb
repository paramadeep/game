describe Judge do
  ...
  it "kills any live position other than two or three live neighbours" do
    [0,1,4,5,6,7,8].each {|lives| expect(Judge.send(:judgement_for,State.alive,lives)).to eq State.dead}
  end

  it "lets any live position with two or three live neighbours lives" do
    [2,3].each {|lives| expect(Judge.send(:judgement_for,State.alive,lives)).to eq State.alive}
  end

  it "brings to life any dead position with exactly three live neighbours" do
    expect(Judge.send(:judgement_for,State.dead,3)).to eq State.alive
  end

  it "any dead position other than with exactly three live neighbours remains dead" do
